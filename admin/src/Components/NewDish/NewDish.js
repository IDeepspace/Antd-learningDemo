...
class NewDish extends Component {
  handleSubmit = e => {
    ...
    console.log(data)
    let unFilled = Object.keys(data).filter(prop => {
      return !data[prop]
    })
    if (unFilled.length === 0) {
      const allData = {
        ...data,
        poster: 'https://avatars3.githubusercontent.com/u/26615866?s=460&v=4'
      }
      axios.post(`http://localhost:3008/dishes`, allData).then(res => {
  ...
  render() {
    ...
    const config = {
      rules: [{ type: 'string', required: true, message: '必填项, config目' }]
    }
    return (
      <NewDishWrap>
        <Form onSubmit={this.handleSubmit}>
          ...
          <Form.Item>
            {getFieldDecorator('price', config)(
              <Input
                prefix={<Icon type="pay-circle-o" style={{ fontSize: 13 }} />}
                placeholder="价格"
              />
            )}
          </Form.Item>
